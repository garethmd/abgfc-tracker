...
| `fixture_selections` / `fixture_selection_players` | **Availability** for an upcoming fixture (`services/selections.py`): one header per fixture (`coaching`, `notes`, cascades) + one row per player with `status` available/unavailable (no reason - in or out is all that's needed). Knowing who *can* play is not a record of who *did* - `appearances` are only written by the result flows and stats never read this. `GET/PUT/DELETE /fixtures/{id}/selection` (replaced whole; 409 once played; player must be in the team's cohort) returns `available`/`unavailable` + `arrival_at`. `GET …/selection/message?date_line=` renders the parents' message in the club's house style listing the available players (`services/messages.py`, pure functions, exact text under test; the frontend never holds the template). Result entry, the live line-up and the matchday PDF use it as a default. |
