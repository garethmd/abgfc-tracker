...
| `fixture_selections` / `fixture_unavailable_players` | **Availability** for an upcoming fixture (`services/selections.py`): one header per fixture (`coaching`, `notes`, cascades) + one `fixture_unavailable_players` row per player marked **out** - everyone else in the squad is available, computed at read time (no status, no reason). Knowing who *can* play is not a record of who *did* - `appearances` are only written by the result flows and stats never read this. `PUT /fixtures/{id}/selection` takes `unavailable_player_ids` (+ coaching, notes; replaced whole; 409 once played); `GET` returns `available`/`unavailable` + `arrival_at`. `GET …/selection/message?date_line=` renders the parents' message in the club's house style listing the available players (`services/messages.py`, pure functions, exact text under test; the frontend never holds the template). Result entry, the live line-up and the matchday PDF use it as a default. |
