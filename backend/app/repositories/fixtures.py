...
from app.models import Appearance, Award, Competition, Fixture, FixtureStatus, MatchEvent
...
class FixtureRepository(BaseRepository[Fixture]):
    ...
    _list_options = (selectinload(Fixture.competition), selectinload(Fixture.opposition))
    ...
    def list_played(self, season_id: int) -> list[Fixture]:
        """Played fixtures with competition loaded - the input to team-record stats."""
        stmt = (
            select(Fixture)
            .where(Fixture.season_id == season_id, Fixture.status == FixtureStatus.PLAYED)
            .options(*self._list_options)
            .order_by(Fixture.kickoff_at, Fixture.id)
        )
        return list(self.db.scalars(stmt))

    def played_fixture_ids(self, season_id: int, competition_type: str | None = None) -> list[int]:
        stmt = select(Fixture.id).where(
            Fixture.season_id == season_id, Fixture.status == FixtureStatus.PLAYED
        )
        if competition_type is not None:
            stmt = stmt.join(Fixture.competition).where(Competition.type == competition_type)
        return list(self.db.scalars(stmt))

    def next_match_number(self, season_id: int) -> int:

        current = self.db.scalar(
            select(func.max(Fixture.match_number)).where(Fixture.season_id == season_id)
        )
